...
public class SearchableListDialog extends DialogFragment implements
    ...
    private void setData(View rootView) {
        ...
        _counter = rootView.findViewById(R.id.counter);
        ...
        _counter.setText(String.format("%d", items.size()));
        ...
        // Si las listas son pequeñas, no mostrar el buscador.
        if (items.size() <= 10) {
    ...
    public boolean onQueryTextChange(String s) {
        if (TextUtils.isEmpty(s)) {
            ...
            ((ArrayAdapter) _listViewItems.getAdapter()).getFilter().filter(s.trim());
        }
        if (null != _onSearchTextChanged) {
            _onSearchTextChanged.onSearchTextChanged(s.trim());
        }
        return true;
    }
